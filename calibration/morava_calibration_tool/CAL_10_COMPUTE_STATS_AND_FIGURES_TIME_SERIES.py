...
for index, row in stationdata_sorted.iterrows():
   ...
   #    continue
   stname = row["River"] + "_" + row["Station"]
   ...
   stname = stname.replace("(", "_")
   stname = stname.replace(" ", "_")
   figfile = os.path.join(SubCatchmentPath, "FIGURES", row['ID'] + "_" + stname + ".png")
   figfile = figfile.replace("__", "_")

   #figfile = os.path.join(SubCatchmentPath, "FIGURES",row['ID']+"_summary.png")
   if not(os.path.exists(figfile)):
    ...
    if os.path.isfile(os.path.join(path_subcatch, "streamflow_simulated_best.csv")):
        ...
        Qsim = pandas.read_csv(os.path.join(path_subcatch, "streamflow_simulated_best.csv"), sep=",", parse_dates=True, index_col=0, header=None,skiprows=1)
    else:
        print ("streamflow_simulated_best.csv missing for catchment" + row['ID'] + "...")
        continue

    Qsim = Qsim.loc[:, 1]  # from dataframe to series
    Qsim = Qsim[Qobs.index[0]:Qobs.index[-1]]
    # Make dataframe with aligned Qsim and Qobs columns
    Q = pandas.concat([Qsim, Qobs], axis=1)  # .reset_index()

    # get first timestamp of Qsim
    timemin = Qsim.index.min().to_pydatetime()
    if Val_Start < timemin: Val_Start = timemin
    ...
    if index == 74:
        iiii =1
    kge = hydroStats.KGE(s=Q_sim_Cal, o=Q_obs_Cal, warmup=WarmupDays)
    ...
    stname = stname.replace("(", "_")
    stname = stname.replace(" ", "_")
    figname = os.path.join(SubCatchmentPath, "FIGURES", row['ID'] + "_"+ stname +".png")
