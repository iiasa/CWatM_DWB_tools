...
if __name__=="__main__":
    ...
    for index, row in stationdata.iterrows():
        ...
        mask.iloc[:ForcingStart_inobs] = True
        mask.iloc[ForcingEnd_inobs:] = True
        ...
        if total_number_vals-4>365*16:    
            ...
            Split = Split.replace(month=1)  # PB replace with first day of month, just for thew beautiness
    ...
    stationdata.to_csv(os.path.join(Qgis_out).replace("\\","/"),',',index=False)
    print ("==================== END ====================")
